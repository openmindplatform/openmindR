...
This is a higher-level function that uses both "bind_questions" and "summarize_comparison" to calculate t-tests and Cohen's d using long-format Assessment data.
...
Now that we have the Ann Miller data in long format we can use our analysis functions to perform mixed models.
...
+ **get_tidy** logical. Get a tidy dataframe with estimates. Also calculates pseudo cohen's d efecct sizes. Default is `FALSE`
